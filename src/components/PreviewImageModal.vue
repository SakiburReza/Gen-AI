...
<template>
  <div class="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center z-50" @click="handleOutsideClick"
    v-if="isOpen">
    <div class="bg-white rounded-lg shadow-lg w-120 max-w-5xl p-6 relative flex flex-col items-center" @click.stop>
      <!-- Close Button -->
      <button @click="close"
        ...
        <div class="w-full flex flex-col items-start">
          ...
          <div style="float: left; text-align: left;" class="w-full">
            <p class="mt-5 text-xl font-bold text-black flex items-start justify-start" style="padding: 10px;">
              Prompt of the Image &nbsp;
              <!-- <img class="h-5 w-5" src="/images/icon/copyIcon.svg" alt="Copy Icon" /> -->
            </p>
            <p class="mt-5 text-xl font-bold text-darkGray bg-tertiary" style="padding: 30px;">
              {{ image.prompt }}
            </p>
          </div>
